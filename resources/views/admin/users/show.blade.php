...
@push('head')
<style>
    .page-head{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:10px;flex-wrap:wrap}
    .page-head h1{font-size:22px;font-weight:800;color:var(--ink);margin:0;letter-spacing:-.01em}
    .page-head .sub{color:var(--muted);font-size:13px;margin-top:3px}

    .st-chip{display:inline-flex;align-items:center;gap:5px;padding:5px 12px;border-radius:100px;font-size:12px;font-weight:600}
    ...
    .back-link{display:inline-flex;align-items:center;gap:8px;color:var(--muted);font-size:13px;font-weight:500;margin-bottom:16px;text-decoration:none;transition:color .2s}
    .back-link:hover{color:var(--primary)}

    .dossier-grid{display:grid;grid-template-columns:1fr 340px;gap:16px}
    @media(max-width:960px){.dossier-grid{grid-template-columns:1fr}}

    .info-card{background:#fff;border-radius:14px;border:1px solid rgba(232,213,192,.55);padding:0;margin-bottom:16px;overflow:hidden}
    .info-card h3{margin:0;padding:16px 22px;font-size:15px;font-weight:700;color:var(--ink);border-bottom:1px solid var(--line)}
    .info-row{display:grid;grid-template-columns:180px 1fr;gap:16px;padding:14px 22px;border-bottom:1px solid rgba(232,213,192,.4)}
    .info-row:last-child{border-bottom:none}
    .info-row .lbl{color:var(--muted);font-size:12.5px}
    .info-row .val{color:var(--ink);font-size:13.5px;font-weight:500}
    .info-row .val a{color:var(--accent);font-weight:600}
    .info-row .val a:hover{color:var(--primary)}

    .side-card{background:#fff;border-radius:14px;border:1px solid rgba(232,213,192,.55);padding:22px;margin-bottom:16px}

    .side-avatar-wrap{display:flex;flex-direction:column;align-items:center;text-align:center}
    .side-avatar{
        width:96px;height:96px;border-radius:22px;
        background:linear-gradient(135deg,var(--accent),var(--secondary));
        color:#fff;display:flex;align-items:center;justify-content:center;
        font-weight:800;font-size:36px;margin-bottom:14px;
        box-shadow:0 8px 20px rgba(107,45,14,.25);
    }
    .side-name{font-size:18px;font-weight:700;color:var(--ink);margin-bottom:4px}
    .side-meta{color:var(--accent);font-size:13px;font-weight:500;margin-bottom:10px}

    .doc-block{
        background:var(--bg-cream);border-radius:12px;padding:18px;text-align:center;
        margin-top:6px;
    }
    .doc-icon{
        width:60px;height:60px;border-radius:14px;background:#fff;color:var(--primary);
        display:flex;align-items:center;justify-content:center;font-size:26px;
        margin:0 auto 10px;
    }
    .doc-name{font-weight:700;color:var(--ink);font-size:13.5px;word-break:break-all}
    .doc-sub{font-size:11.5px;color:var(--muted);margin-top:2px;margin-bottom:12px}
    .doc-btn{
        display:inline-flex;align-items:center;gap:6px;background:#fff;color:var(--primary);
        border:1px solid var(--line);padding:8px 14px;border-radius:9px;font-size:12.5px;font-weight:600;
        text-decoration:none;transition:all .2s;
    }
    .doc-btn:hover{background:var(--primary);color:#fff;border-color:var(--primary)}

    .decision{margin-top:4px}
    .decision h4{font-size:14px;font-weight:700;color:var(--ink);margin-bottom:12px}
    .btn-full{
        display:flex;align-items:center;justify-content:center;gap:8px;
        width:100%;padding:12px 16px;border:none;border-radius:11px;
        font-weight:700;font-size:13.5px;cursor:pointer;font-family:inherit;transition:all .2s;margin-bottom:10px;
    }
    .btn-validate{background:linear-gradient(135deg,#4A7C59,#2D5A3C);color:#fff;box-shadow:0 6px 14px rgba(74,124,89,.25)}
    .btn-validate:hover{transform:translateY(-1px);box-shadow:0 8px 18px rgba(74,124,89,.35)}
    .btn-reject{background:#FEE4E0;color:#A8332A}
    .btn-reject:hover{background:#C94A3A;color:#fff}
    .reason-input{
        width:100%;border:1px solid var(--line);border-radius:11px;padding:11px 14px;
        font-size:13px;font-family:inherit;resize:vertical;min-height:70px;margin-bottom:10px;
        background:var(--bg-cream);
    }
    .reason-input:focus{outline:none;border-color:var(--accent);background:#fff}

    .suspend-toggle{
        display:flex;align-items:center;justify-content:center;gap:8px;
        width:100%;padding:11px 14px;border-radius:10px;font-weight:600;font-size:13px;
        cursor:pointer;font-family:inherit;border:none;transition:all .2s;
    }
    .suspend-toggle.suspend{background:#FDF3DD;color:#A77914}
    .suspend-toggle.suspend:hover{background:#E8A020;color:#fff}
    .suspend-toggle.activate{background:#E7F5EC;color:#2B6B43}
    .suspend-toggle.activate:hover{background:#4A7C59;color:#fff}
    .delete-btn{background:#FEE4E0;color:#A8332A;margin-top:6px}
    .delete-btn:hover{background:#C94A3A;color:#fff}

    .pills-row{display:flex;flex-wrap:wrap;gap:6px;margin-top:6px}
...
<a href="{{ route('admin.users.index', ['validation' => $isPending ? 'pending' : null, 'role' => $isArtisan ? 'artisan' : 'client']) }}" class="back-link">
...
<div class="dossier-grid">
            ...
            <div class="info-row">
                ...
                <span class="val">
                    <a href="{{ asset('storage/'.$p->proof_document) }}" target="_blank">
                        {{ basename($p->proof_document) }} <i class="bi bi-box-arrow-up-right" style="font-size:11px"></i>
                    </a>
                </span>
            </div>
            @endif
    ...
    <aside>
        ...
        <div class="side-card">
            <div class="decision">
                ...
                <form action="{{ route('admin.users.reject-artisan', $user) }}" method="POST" style="margin:0">
                    @csrf @method('PATCH')
                    <textarea name="rejection_reason" class="reason-input" placeholder="Motif de refus (optionnel)..."></textarea>
        ...
        <div class="side-card">
            ...
            @if(!$user->isAdmin())
            <form action="{{ route('admin.users.destroy', $user) }}" method="POST" style="margin:0;margin-top:8px" onsubmit="return confirm('Supprimer définitivement cet utilisateur ?')">
                @csrf @method('DELETE')
