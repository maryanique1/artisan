...
<style>
    ...
    .search-pill{
        ...
        border:1px solid var(--line);border-radius:10px;padding:9px 14px;min-width:260px;
    }
    ...
    .seg .seg-btn{
        ...
        padding:7px 16px;border-radius:9px;font-size:12.5px;font-weight:600;cursor:pointer;
        font-family:inherit;transition:all .2s;text-decoration:none;display:inline-flex;align-items:center;gap:5px;
    }
    ...
    .mini-select,.mini-input{
        ...
        font-size:12.5px;font-family:inherit;color:var(--ink);outline:none;min-width:140px;
    }
    ...
    .act-btn{
        ...
        font-size:12px;font-weight:600;border:none;cursor:pointer;text-decoration:none;font-family:inherit;
        transition:all .2s;
    }
    ...
    .pager{
        ...
        padding:14px 18px;border-top:1px solid var(--line);gap:10px;flex-wrap:wrap;
    }
    ...
    .pager .page-link{
        ...
        padding:6px 12px;font-size:12.5px;border-radius:8px !important;
    }
    ...
    .cell-user .em{font-size:11.5px;color:var(--muted)}
    ...
    .tip{font-size:12.5px;color:var(--muted);display:inline-flex;align-items:center;gap:6px}
</style>
@endpush
...
<div class="card-feg">
    ...
    <form method="GET" class="filters">
        ...
        @if($pageMode === 'pending')
            <div class="seg">
                <a class="seg-btn {{ !request('period') ? 'active' : '' }}" href="{{ request()->fullUrlWithQuery(['period'=>null]) }}">Tous ({{ $users->total() }})</a>
                <a class="seg-btn {{ request('period') === 'today' ? 'active' : '' }}" href="{{ request()->fullUrlWithQuery(['period'=>'today']) }}">Aujourd'hui ({{ $countToday ?? 0 }})</a>
                <a class="seg-btn {{ request('period') === 'week' ? 'active' : '' }}" href="{{ request()->fullUrlWithQuery(['period'=>'week']) }}">Cette semaine ({{ $countWeek ?? 0 }})</a>
            </div>
        ...
        @else
            <div class="seg">
                <a class="seg-btn {{ !request('status') ? 'active' : '' }}" href="{{ request()->fullUrlWithQuery(['status'=>null]) }}">Tous</a>
                <a class="seg-btn {{ request('status') === 'active' ? 'active' : '' }}" href="{{ request()->fullUrlWithQuery(['status'=>'active']) }}">Actifs</a>
                <a class="seg-btn {{ request('status') === 'suspended' ? 'active' : '' }}" href="{{ request()->fullUrlWithQuery(['status'=>'suspended']) }}">Suspendus</a>
            </div>
            <div class="filters-right">
                ...
                @endif
                <input type="text" name="city" class="mini-input" placeholder="Filtrer par ville..." value="{{ request('city') }}">
            </div>
        @endif
    </form>
    ...
    <table class="table-feg">
        <thead>
            <tr>
                @if($pageMode === 'pending')
                    <th>Artisan</th><th>Catégorie</th><th>Zone</th><th>Document</th><th>Soumis le</th><th>Actions rapides</th>
                @elseif($pageMode === 'artisans')
                    <th>Artisan</th><th>Catégorie</th><th>Zone</th><th>Inscrit</th><th>Avis</th><th>Statut</th><th>Actions</th>
                @elseif($pageMode === 'clients')
                    <th>Client</th><th>Téléphone</th><th>Ville</th><th>Inscrit le</th><th>Statut</th><th>Actions</th>
                @else
        ...
        <tbody>
            ...
            <tr
                @if($pageMode === 'pending')
                    onclick="window.location='{{ route('admin.users.show', $user) }}'" style="cursor:pointer"
                @endif
            >
                {{-- ══ Colonne principale (nom + email) ══ --}}
                <td>
                    <div class="cell-user">
                        <div class="avatar-sq p{{ ($i % 5) + 1 }}">{{ mb_strtoupper(mb_substr($user->first_name ?? $user->name, 0, 1)) }}</div>
                        <div>
                ...
                @if($pageMode === 'pending')
                    ...
                    <td onclick="event.stopPropagation()">
                        <div style="display:flex;gap:6px;flex-wrap:wrap">
                            <a href="{{ route('admin.users.show', $user) }}" class="act-btn act-view"><i class="bi bi-eye"></i> Voir dossier</a>
                ...
                @elseif($pageMode === 'artisans')
                    ...
                    <td>{{ $p?->ville }}@if($p?->quartier) · {{ $p->quartier }}@endif</td>
                    <td style="color:var(--muted)">{{ $user->created_at->translatedFormat('M Y') }}</td>
                    <td>
                    ...
                    <td>
                        <div style="display:flex;gap:6px">
                            <a href="{{ route('admin.users.show', $user) }}" class="act-btn act-view"><i class="bi bi-eye"></i> Voir</a>
                ...
                @else
                    <td><span class="st-chip" style="background:var(--bg-cream);color:var(--primary)">{{ ucfirst($user->role) }}</span></td>
                    <td>{{ $user->ville ?: '—' }}</td>
                    <td style="color:var(--muted)">{{ $user->created_at->format('d/m/Y') }}</td>
                    <td>
                        @if($user->is_active)<span class="st-chip ok">Actif</span>@else<span class="st-chip danger">Suspendu</span>@endif
                    </td>
                    <td>
                        <a href="{{ route('admin.users.show', $user) }}" class="act-btn act-view"><i class="bi bi-eye"></i> Voir</a>
                    </td>
            ...
            @empty
                <tr><td colspan="7" class="empty-row" style="padding:40px;text-align:center;color:var(--muted)">
    ...
    <div class="pager">
        <div class="pager-info">
            @if($users->total() > 0)
                Affichage {{ $users->firstItem() }}–{{ $users->lastItem() }} sur {{ number_format($users->total(), 0, ',', ' ') }}
