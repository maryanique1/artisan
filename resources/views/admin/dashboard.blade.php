...
<style>
    ...
    .sc{
        background:#fff;border:1px solid rgba(232,213,192,.55);border-radius:14px;
        padding:14px 16px 14px;position:relative;overflow:hidden;
        transition:transform .2s,box-shadow .2s;
    }
    .sc:hover{transform:translateY(-1px);box-shadow:0 4px 14px rgba(44,26,14,.05)}
    .sc::before{content:"";position:absolute;top:0;left:0;right:0;height:3px;background:var(--c,var(--primary));border-radius:14px 14px 0 0}
    .sc-head{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:10px}
    .sc-ico{
        width:34px;height:34px;border-radius:9px;background:var(--ico-bg,#FDF3E6);color:var(--c,var(--primary));
        display:flex;align-items:center;justify-content:center;font-size:15px;
    }
    .sc-badge{font-size:10.5px;font-weight:600;padding:3px 8px;border-radius:100px;display:inline-flex;align-items:center;gap:3px;white-space:nowrap}
    .sc-badge.ok{background:#E7F5EC;color:#2B6B43}
    .sc-badge.warn{background:#FDF3DD;color:#A77914}
    .sc-badge.danger{background:#FEE4E0;color:#A8332A}
    .sc-val{font-size:24px;font-weight:800;color:var(--ink);line-height:1;letter-spacing:-.02em;margin-bottom:4px}
    .sc-label{font-size:12.5px;font-weight:600;color:var(--ink)}
    .sc-sub{font-size:11px;color:var(--muted);margin-top:1px}

    /* variants */
    .sc.c-primary{--c:var(--primary);--ico-bg:#FDF3E6}
    .sc.c-accent{--c:var(--accent);--ico-bg:#FAEEDD}
    .sc.c-gold{--c:var(--warning);--ico-bg:#FDF3DD}
    .sc.c-danger{--c:var(--danger);--ico-bg:#FBE8E4}

    /* ═════ 2-COL LAYOUT ═════ */
    .grid-2{display:grid;grid-template-columns:1.7fr 1fr;gap:14px;margin-bottom:14px}
    .grid-3{display:grid;grid-template-columns:1.7fr 1fr;gap:14px}
    ...
    .cell-user .name{font-weight:600;color:var(--ink);font-size:13.5px;line-height:1.3}
    .cell-user .mail{font-size:12px;color:var(--muted)}

    /* ═════ DONUT ═════ */
    .donut-wrap{display:flex;flex-direction:column;align-items:center;padding:8px 18px 16px;position:relative}
    .donut-box{position:relative;width:150px;height:150px;margin:4px auto 14px}
    .donut-center{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;pointer-events:none}
    .donut-center .val{font-size:22px;font-weight:800;color:var(--ink);line-height:1}
    .donut-center .lbl{font-size:10.5px;color:var(--muted);margin-top:3px}

    .legend{width:100%;display:flex;flex-direction:column;gap:7px;padding:0 4px}
    ...
    .legend-pct{font-size:11.5px;color:var(--muted);width:38px;text-align:right}

    /* ═════ ACTIVITY FEED ═════ */
    .feed{padding:4px 18px 12px}
    .feed-item{display:flex;gap:11px;padding:10px 0;border-bottom:1px solid rgba(232,213,192,.5)}
    .feed-item:last-child{border-bottom:none}
    .feed-ico{
        width:30px;height:30px;border-radius:8px;background:var(--bg-cream);color:var(--accent);
        display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0;
    }
    .feed-body{flex:1;min-width:0}
    .feed-body .t{font-size:12.5px;color:var(--ink);line-height:1.4}
    .feed-body .t b{font-weight:700}
    .feed-body .m{font-size:10.5px;color:var(--muted);margin-top:2px}

    .empty-row{padding:22px;text-align:center;color:var(--muted);font-size:12.5px}
    ...
    <div class="sc c-danger">
        <div class="sc-head">
            ...
            @if($stats['pending_reports'] > 0)
                <span class="sc-badge danger">{{ $stats['pending_reports'] }} non traité{{ $stats['pending_reports'] > 1 ? 's' : '' }}</span>
            @endif
        </div>
...
<div class="grid-2">
    ...
    <div class="card-feg">
        <div class="card-head">
            ...
            <span class="chip warn">{{ $stats['pending_validations'] }} dossier{{ $stats['pending_validations'] > 1 ? 's' : '' }}</span>
        </div>
        ...
        <table class="table-feg">
            ...
            <tbody>
                ...
                <tr onclick="window.location='{{ route('admin.users.show', $p->user) }}'" style="cursor:pointer">
                    <td>
                        <div class="cell-user">
                            <div class="avatar-sq p{{ ($i % 5) + 1 }}">{{ mb_strtoupper(mb_substr($p->user->first_name ?? $p->user->name, 0, 1)) }}</div>
                            <div>
...
<div class="grid-3">
    ...
    <div class="card-feg">
        <div class="card-head">
            ...
            @if($recentReports->count())
                <span class="chip danger">{{ $recentReports->count() }} non traité{{ $recentReports->count() > 1 ? 's' : '' }}</span>
            @endif
        </div>
        @if($recentReports->count())
        <div style="overflow-x:auto">
        <table class="table-feg">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div class="cell-user">
                            <div class="avatar-sq p{{ ($i % 5) + 1 }}" style="width:32px;height:32px;font-size:12px">
                    ...
                    <td>
                        <span class="chip neutral">{{ $r->target_type === 'publication' ? 'Publication' : 'Profil' }}</span>
                    </td>
                    ...
                    <td>
                        <a href="{{ route('admin.reports.index') }}" class="btn-feg-outline" style="padding:5px 10px;font-size:12px">
                            <i class="bi bi-eye"></i>
                        </a>
                    </td>
                </tr>
    ...
    <div class="card-feg">
        <div class="card-head">
            ...
            <a href="{{ route('admin.users.index') }}" style="font-size:12.5px;color:var(--accent);font-weight:600">Tout voir →</a>
        </div>
        <div class="feed">
