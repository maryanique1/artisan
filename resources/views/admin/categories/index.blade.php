...
@push('head')
    <style>
        ...
        .btn-add:hover {
            ...
            box-shadow: 0 6px 18px rgba(107, 45, 14, .25);
            color: #fff
        }
        ...
        .cat-row {
            display: grid;
            grid-template-columns: 72px 2fr 1.5fr 160px 110px auto;
            gap: 18px;
            align-items: center;
            padding: 14px 22px;
        ...
        @media(max-width:900px) {
            ...
            .cat-actions {
                ...
                border-top: 1px solid rgba(232, 213, 192, .5);
                padding-top: 10px;
                gap: 8px;
            }
        }
...
@section('content')
    ...
    <div class="page-head">
        <div>
            <h1>Métiers</h1>
            <div class="sub">{{ $categories->where('is_active', true)->count() }}
                métier{{ $categories->where('is_active', true)->count() > 1 ? 's' : '' }}
                actif{{ $categories->where('is_active', true)->count() > 1 ? 's' : '' }} sur {{ $categories->count() }}
            </div>
        </div>
    ...
    <div class="card-feg">
        ...
        <div class="filters" style="flex-direction:column;gap:10px;align-items:flex-start">
            ...
            <div class="seg">
                ...
                <a class="seg-btn {{ !request('filter') ? 'active' : '' }}"
                    href="{{ request()->fullUrlWithQuery(['filter' => null]) }}">Tous</a>
                <a class="seg-btn {{ request('filter') === 'active' ? 'active' : '' }}"
                    href="{{ request()->fullUrlWithQuery(['filter' => 'active']) }}">Actifs</a>
                <a class="seg-btn {{ request('filter') === 'inactive' ? 'active' : '' }}"
                    href="{{ request()->fullUrlWithQuery(['filter' => 'inactive']) }}">Désactivés</a>
            </div>
        </div>
        ...
        <div class="cat-list">
            @forelse($categories as $c)
                ...
                <div class="cat-row {{ $c->is_active ? '' : 'inactive' }}">
                    ...
                    <div class="cat-stats">
                        <div class="count">{{ $c->artisan_profiles_count }}
                            <small>artisan{{ $c->artisan_profiles_count > 1 ? 's' : '' }} · {{ $pct }}%</small></div>
                    ...
                    <div class="cat-actions">
                        <button class="cat-btn edit" data-bs-toggle="modal" data-bs-target="#editModal{{ $c->id }}"
                            title="Modifier">
                        ...
                        <button class="cat-btn del" {{ $c->artisan_profiles_count > 0 ? 'disabled' : '' }}
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <select name="parent_id" class="form-select" required>
                                    <option value="">-- Choisir une branche --</option>
                                    @foreach($branches as $b)
                                        <option value="{{ $b->id }}" {{ $c->parent_id == $b->id ? 'selected' : '' }}>{{ $b->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <label class="upload-zone">
                                    <input type="file" name="image" accept="image/jpeg,image/png,image/webp"
                                        onchange="previewImage(this,'editPreview{{ $c->id }}')">
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px">
        <div class="modal-content" style="border-radius:16px;border:none;box-shadow:0 20px 60px rgba(44,26,14,.15)">
            <div class="modal-body" style="padding:32px 28px;text-align:center">
                <div
                    style="width:56px;height:56px;border-radius:14px;background:#FEE4E0;display:flex;align-items:center;justify-content:center;margin:0 auto 18px;font-size:24px;color:#C94A3A">
                ...
                <p style="color:var(--muted);font-size:13.5px;margin-bottom:24px">
                    Vous allez supprimer <strong id="deleteTargetName" style="color:var(--ink)"></strong>. Cette action
                    est irréversible.
                </p>
                <div style="display:flex;gap:10px;justify-content:center">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"
                        style="border-radius:10px;font-weight:600;padding:10px 22px">
                        Annuler
                    </button>
                    <form id="deleteForm" method="POST" style="margin:0">
                        @csrf @method('DELETE')
                        <button type="submit"
                            style="background:#C94A3A;color:#fff;border:none;border-radius:10px;font-weight:700;padding:10px 22px;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;gap:7px">
